...
static unsigned int _get_pixel      (fb_info_t *fb, int x, int y);
static unsigned int _get_pixel_1bpp (fb_info_t *fb, int x, int y);   // ssd3306 OLED
unsigned int         get_pixel      (fb_info_t *fb, int x, int y);
...
int         make_draw_text (char *img_buf, int w, int h, int bpp,
                     int f_color, int b_color, int scale, char *fmt, ...);
...
static unsigned int _get_pixel (fb_info_t *fb, int x, int y)
{
    ...
    if (fb->is_bgr) {
        c.bits.b = *(fb->data + offset);    offset++;
        c.bits.g = *(fb->data + offset);    offset++;
        c.bits.r = *(fb->data + offset);    offset++;
    } else {
        c.bits.r = *(fb->data + offset);    offset++;
        c.bits.g = *(fb->data + offset);    offset++;
        c.bits.b = *(fb->data + offset);    offset++;
    ...
    if (fb->bpp == 32)  c.bits.a = 0xFF;

    return c.uint;
}

//-----------------------------------------------------------------------------
static unsigned int _get_pixel_1bpp (fb_info_t *fb, int x, int y)
{
    int offset = ((y / 8) * fb->w) + (x % fb->w);
    int shift  =  (y % 8);
    unsigned char data = *(fb->data + offset);

    return (data & (0x01 << shift)) != 0 ? -1 : 0;
}
...
unsigned int get_pixel (fb_info_t *fb, int x, int y)
{
    ...
    bit_color = (fb->bpp != 1) ? _get_pixel (fb, x, y) : _get_pixel_1bpp (fb, x, y);
...
static void _put_pixel (fb_info_t *fb, int x, int y, int color)
{
    ...
    if (fb->bpp == 32)  *(fb->data + offset) = 0xFF;
}
                    ...
                    int f_color, int b_color, int scale, char *fmt, ...)
{
    ...
    return draw_text (&img_fb, 0, 0, f_color, b_color, scale, fmt);
}
